...
if __name__ == '__main__':
    ...
    path = 'results/mBlip_prompt_results.tsv'

    df = pd.read_csv(path, sep='\t')

    for lang in range(len(df.index)):
        gens[df.iloc[lang]['lang']].append(df.iloc[lang]['simple_caption'])
        refs[df.iloc[lang]['lang']].append(df.iloc[lang]['caption_reference_description'])
    for key in gens:
    ...
    for lang in range(len(df.index)):
        gens[df.iloc[lang]['lang']].append(df.iloc[lang]['context_caption'])
        refs[df.iloc[lang]['lang']].append(df.iloc[lang]['caption_reference_description'])
    for key in gens:
        gen_ref_dict[key].append(gens[key])
        gen_ref_dict[key].append(refs[key])
   ...
   # Set up dataframes
    ...
    for lang in range(len(df.index)):
        gens[df.iloc[lang]['lang']].append(df.iloc[lang]['context_transl_caption'])
        refs[df.iloc[lang]['lang']].append(df.iloc[lang]['caption_reference_description'])
    for key in gens:
        gen_ref_dict[key].append(gens[key])
        gen_ref_dict[key].append(refs[key])
